...
async function verifyKey(key) {
  ...
  if (!cleanKey) {
    return { passed: false, msg: 'Please enter your license key.' };
  }
  ...
  try {
    ...
    removeKey();
    return { passed: false, msg: res.data.message || 'Key is invalid.' };
  } catch (err) {
    if (err.response && err.response.data && err.response.data.message) {
      const msg = err.response.data.message;
      removeKey();
      if (msg === 'key_expired') return { passed: false, msg: 'This key has expired.' };
      if (msg === 'hwid_mismatch') return { passed: false, msg: 'Key is locked to another computer.' };
      if (msg === 'key_inactive') return { passed: false, msg: 'This key is currently deactivated.' };
      if (msg === 'key_not_found') return { passed: false, msg: 'License key not found.' };
    ...
    return { passed: false, msg: 'Could not connect to server. Check your internet connection.' };
  }
}
